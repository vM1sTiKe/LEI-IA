...
(DEFUN get-node-string (node)
  (LET (
    ...
    (PROGN
      (CONCATENATE 'string
        (FORMAT NIL "=============================~%")
        (FORMAT NIL " ~a ~a|~a|~a|~a|~a|~a   points: ~a~%" (IF (= playing-row 0) "->" "  ") cell-0-0 cell-0-1 cell-0-2 cell-0-3 cell-0-4 cell-0-5 (get-row-points node 0))
        (FORMAT NIL "    -----------~%")
        (FORMAT NIL " ~a ~a|~a|~a|~a|~a|~a   points: ~a~%" (IF (= playing-row 1) "->" "  ") cell-1-0 cell-1-1 cell-1-2 cell-1-3 cell-1-4 cell-1-5 (get-row-points node 1))
        (FORMAT NIL "=============================~%")
      )
    )
  )
